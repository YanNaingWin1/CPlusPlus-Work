...
class CensorColorPicker : public ColorPicker
{
  ...
  private:
    ...
    void blockify(PNG* blockyimg, unsigned int x_counter, unsigned int y_counter);   // block-ifies the original image given and returns it
    //void assignAvgColor(PNG* img, double h, double s, double l, double a, unsigned int start_val_x, unsigned int start_val_y); // calculates average and assign the color results to the block
};
