...
T Stack<T>::Pop() {
  ...
  T ret = items[Size() - 1];
  num_items--;
  double dec_size = (double) Size();
  double dec_capa = (double) Capacity();
  double dec_shri = (double) SHRINKRATE;
  double dec_expa = (double) EXPANSIONFACTOR;
  if ((dec_size/dec_capa) < (1.0/dec_shri)) {
    if (((double)max_items)/dec_expa > DEFAULTCAPACITY) {
      Resize(((double)max_items)/dec_expa);
    } else {
