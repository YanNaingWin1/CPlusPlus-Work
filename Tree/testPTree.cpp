...
TEST_CASE("PTree::FlipHorizontal_2x1", "[weight=1][part=ptree]") {
  //cout << "Testing PTree FlipHorizontal_2x1..." << endl;
  ...
  *somepixel = HSLAPixel(0, 0.25, 0.75);

  somepixel = myfancypng.getPixel(1, 0);
  *somepixel = HSLAPixel(240, 0.4, 0.35);

  myfancypng.writeToFile("images/fliphorizontal-original-2x1.png");

  // call the tree constructor
  PTree tree(myfancypng);

  // FlipHorizontal tree
  tree.FlipHorizontal();

  PNG soln_image;
  soln_image.resize(2, 1);

  HSLAPixel *solnpixel;

  solnpixel = soln_image.getPixel(0, 0);
  *solnpixel = HSLAPixel(240, 0.4, 0.35);

  solnpixel = soln_image.getPixel(1, 0);
  *solnpixel = HSLAPixel(0, 0.25, 0.75);

  soln_image.writeToFile("images/fliphorizontal-solution-2x1.png");

  PNG output = tree.Render();

  output.writeToFile("images/fliphorizontal-mix-2x1.png");
